...
namespace MagicLeapTools
{
    ...
    public class Playspace : MonoBehaviour
    {
        ...
        private void DrawPerimeter_HandleTriggerDown(byte controllerId, float triggerValue)
        {
            ...
            // get the raycast hit from the controller beam
            RaycastHit hit;

            if(Physics.Raycast(MLInput.GetController(controllerId).Position, MLInput.GetController(controllerId).Orientation * Vector3.forward, out hit))
            {
                //corner details:

                Vector3 cornerLocation = new Vector3(hit.point.x, _roomVerticalCenter, hit.point.z);
                ...
                if (!loopComplete)
                {
                    ...
                    // Get controller bumper input
                    MLInput.OnControllerButtonDown += DrawPerimeter_HandleOnButtonDown;
                }
                else
                {
                    //visualize boundry:
        ...
        private void DrawPerimeter_HandleOnButtonDown(byte controllerId, MLInput.Controller.Button button)
        {
            if (button == MLInput.Controller.Button.Bumper)
            {
                // remove the last corner
                Destroy(_plottedCorners[_plottedCorners.Count - 1].gameObject);
                _plottedCorners.RemoveAt(_plottedCorners.Count - 1);
        ...
        private void Idle_Enter()
        {
            ...
            MLInput.OnControllerButtonDown += Idle_HandleControllerButtonDown;
        }

        private void Idle_HandleControllerButtonDown(byte controllerId, MLInput.Controller.Button button)
        {
